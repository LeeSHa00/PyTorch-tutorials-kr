...
예를들어, 순환 커널을 직교하게 (orthogonal) 만들어서 특이값 (singular values)을 제어할 수 있습니다.

순환 모델을 정규화하는 또 다른 방법은 비중 정규화를 활용한 방법입니다 :
...
가중치 및 스펙트럼 정규화의 경우 원래 매개변수를 norm으로 나눕니다.
...
###############################################################################
# 이 구현은 정확하고 독립적이지만 몇가지 문제를 가지고 있습니다.:
#
...
A = layer.weight
assert torch.allclose(A, A.T)  # A는 대칭
print(A)                       # 출력 확인
...
# 행렬 지수는  행렬도 직교 행렬에 매핑합니다. 이 두 가지 사실을 사용하여
